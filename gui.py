...
class Ui_iPhaser(QMainWindow):
    ...
    def setupUi(self, cfg):
        ...
        Vpercent = 100 / len(Vblocks)
        Hpercent = 100 / len(Hblocks)
        for i in Vblocks:
            self.setVLayout(i, Vpercent)
        # for i in Hblocks:
        #     self.setHLayout(i, Hpercent)
    ...
    def windowResized(self, event):
        ...
        videoWidth = width - 500
        videoHeight = height - 250
    ...
    def update_image(self, cv_img):
        ...
        bytes_per_line = ch * w
        convert_to_Qt_format = QtGui.QImage(rgb_image.data, w, h, bytes_per_line, QtGui.QImage.Format_RGB888)
        p = convert_to_Qt_format.scaled(self.size.width(), self.size.height(),
                                        Qt.KeepAspectRatio)  # TODO: Change size of display
        p = QPixmap.fromImage(p)
        # print('done')
        # print(self.INIT)
        self.DisplayVideo.setPixmap(p)
